...
fn run_holdings_in_dir(date: &str, conn: &mut diesel::SqliteConnection) -> String {
    ...
    holdings(&date.to_string(), "gaap", conn).unwrap();
...
/// Parse CSV into rows. Returns data rows (header excluded).
...
fn test_holdings_single_lot() {
    ...
    // Detail row: [wallet, acquisition_date, btc, undisposed_btc, usd_basis, usd_fair_value]
    assert_eq!(rows[0][1], "2024-01-15T00:00:00");
    assert!(rows[0][2].starts_with("1"), "Btc should be 1: got {}", rows[0][2]);
    assert!(rows[0][4].contains("40000"), "Should reference $40k basis: got {}", rows[0][4]);
}
...
fn test_holdings_excludes_future_acquisitions() {
    ...
    assert_eq!(rows[0][1], "2024-01-01T00:00:00");
    assert!(rows[0][4].contains("15000"), "Basis for 0.5 BTC at $30k = $15k: got {}", rows[0][4]);
}
...
fn test_holdings_adjusts_for_dispositions() {
    ...
    // undisposed_btc (index 3) should be 0.50
    assert!(rows[0][3].starts_with("0.5"), "Should show 0.5 BTC undisposed: got {}", rows[0][3]);
}
...
fn test_holdings_subsequent_disposals_added_back() {
    ...
    let rows = parse_holdings_csv(&content);
    assert_eq!(rows.len(), 2, "Expected 2 rows: {}", content);
    // undisposed_btc (index 3) should be 1 (full lot since disposal is in the future)
    assert!(rows[0][3].starts_with("1"), "Should show full 1.0 BTC before disposal date: got {}", rows[0][3]);
}
...
fn test_holdings_fully_disposed_lot_excluded() {
    ...
    // Totals row should show 0 for all values (wallet col shifts indices)
    assert_eq!(rows[0].get(2).unwrap(), "0", "Total BTC should be 0");
    assert_eq!(rows[0].get(3).unwrap(), "0", "Total undisposed BTC should be 0");
}
